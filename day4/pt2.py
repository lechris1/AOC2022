...
def checkCompleteOverlap(pair1, pair2):
    ...
    if (
            (
                int(pair1[0]) <= int(pair2[0]) 
                and (int(pair1[1]) >= int(pair2[1]) or (int(pair1[1]) <= int(pair2[1]) and int(pair1[1]) >= int(pair2[0])))
            )
            or 
            (
                int(pair2[0]) <= int(pair1[0]) 
                and (int(pair2[1]) >= int(pair1[1]) or (int(pair2[1]) <= int(pair1[1]) and int(pair2[1]) >= int(pair1[0])))
            )
        ):
        isCompleteOverlap = 1
